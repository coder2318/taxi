...
class InsertTranslations extends Command
{
     ...
     * @var string
     */
    protected $description = 'Create to db localizations';
    ...
    public function handle()
    {
        ...
        $data = $this->rec1($array);
        print_r($data);
    }

    public $keys  = [];
    public $group  = 'installer_messages';

    public function rec1($data, $lastKey = false){
        foreach($data as $index => $item){

            $this->keys = array_merge( $this->keys, [$index]);

            if(is_array($item)){
                $arrayKeys = array_keys($item);
                $lastKey = array_pop($arrayKeys);
                $this->rec1($item, $lastKey);
            }else {

                $ok = [//DB ga create shu joyda
                    'text' => $item,
